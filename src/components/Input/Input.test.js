...
describe("Input", () => {
  it("renders", () => {
    const wrapper = createWrapperWithTheme(<Input />);
    expect(wrapper.find("input").length).toBe(1);
  });
  it("renders with a small class", () => {
    const wrapper = createWrapperWithTheme(<Input size="small" />);
    expect(wrapper.find("input.sm")).toBeTruthy();
  });
  it("renders with a primary class", () => {
    const wrapper = createWrapperWithTheme(<Input type="primary" />);
    expect(wrapper.find("input.primary")).toBeTruthy();
  });
  it("handles onChange event", () => {
    const spyOnChange = jest.fn();
    const wrapper = createWrapperWithTheme(<Input onChange={spyOnChange} />);
